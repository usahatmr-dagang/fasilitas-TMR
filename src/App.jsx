...
export default function App() {
  ...
  const availableDatesList = useMemo(() => {
    const todayStr = getTodayString();
    const dataKedepan = sewaList.filter(sewa => sewa.tanggal_sewa >= todayStr && sewa.status_pembayaran !== 'Ditutup');
    const groups = dataKedepan.reduce((acc, curr) => {
      if (!acc[curr.tanggal_sewa]) acc[curr.tanggal_sewa] = 0;
      acc[curr.tanggal_sewa]++;
      return acc;
    }, {});
    const totalCount = dataKedepan.length;
    return {
      total: totalCount,
      dates: Object.keys(groups).sort((a, b) => new Date(a) - new Date(b)).map(date => ({
        date,
        count: groups[date]
      }))
    };
...
Terima kasih.`;
  ...
  const handleBlockLokasi = async (e) => {
      e.preventDefault();
      const currentDateTime = new Date().toLocaleString('id-ID', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' });
  ...
  const handleUpdateRecordInline = async (record, updates, successMsg) => {
      try {
          ...
          <div className="fixed inset-0 bg-[#022c22]/60 backdrop-blur-sm z-[50] flex justify-end transition-all" onClick={() => setSelectedRecord(null)}>
            <div className="w-full max-w-md bg-slate-50 h-full shadow-[-10px_0_50px_rgba(2,44,34,0.15)] flex flex-col animate-slide-in-right p-6" onClick={e => e.stopPropagation()}>
                ...
                <div className="bg-white rounded-3xl p-8 text-center shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 flex-1 flex flex-col justify-center">
                    ...
                    <button type="button" onClick={() => handleDeleteRecordInline(selectedRecord, 'Lokasi berhasil dibuka kembali!')} className="w-full bg-emerald-600 text-white font-bold py-4 rounded-2xl shadow-[0_8px_20px_-4px_rgba(16,185,129,0.3)] hover:bg-emerald-700 hover:-translate-y-0.5 transition-all duration-200">
                  ...
                  <>
                         ...
                         <label className="relative inline-flex items-center cursor-pointer">
                             <input 
                                 ...
                                 onChange={(e) => {
                                     ...
                                     handleUpdateRecordInline(selectedRecord, { listrik_tambahan: isChecked, akses_upload_listrik: isChecked, total_biaya: biayaLok + biayaLis }, 'Status listrik diperbarui.');
                                 }} 
                             />
                          ...
                          <div className="bg-white rounded-3xl p-5 shadow-[0_4px_20px_rgba(4,120,87,0.03)] border border-amber-200 relative overflow-hidden">
                              ...
                              <div className="flex items-center justify-between mb-4 border-b border-amber-100 pb-3">
                                  ...
                                  <label className={`flex items-center cursor-pointer px-3 py-1.5 rounded-xl border transition-all duration-200 ${selectedRecord.akses_upload_listrik ? 'bg-amber-100 border-amber-300 shadow-sm' : 'bg-slate-50 border-slate-200'}`}>
                                      ...
                                      <input type="checkbox" checked={selectedRecord.akses_upload_listrik || false} onChange={(e) => handleUpdateRecordInline(selectedRecord, { akses_upload_listrik: e.target.checked })} className="w-4 h-4 text-amber-600 rounded border-slate-300 focus:ring-amber-500 cursor-pointer" />
                                  </label>
                              </div>
                      ...
                      <div className="bg-white rounded-3xl p-5 shadow-[0_4px_20px_rgba(4,120,87,0.03)] border border-slate-100">
                         ...
                         {selectedRecord.status_pembayaran === 'Menunggu Verifikasi' && selectedRecord.bukti_transfer && (
                             <button type="button" onClick={() => handleUpdateRecordInline(selectedRecord, { status_pembayaran: 'Sudah Transfer' }, 'Pembayaran berhasil diverifikasi!')} className="w-full bg-emerald-600 text-white font-bold py-3 rounded-xl hover:bg-emerald-700 shadow-md hover:shadow-lg transition-all duration-200 mb-3 flex justify-center items-center"><CheckCircle2 size={16} className="mr-2"/> Verifikasi Valid</button>
                         )}
  ...
  const handleAddMasterLokasi = async (e) => {
    ...
    try {
      ...
      setToast({ show: true, msg: 'Gagal menambah lokasi!', type: 'error' });
    }
  };

  const handleDeleteMasterLokasi = async (id) => {
    try {
      await deleteDoc(doc(db, 'masterLokasi', id));
    ...
    <div className="flex h-screen bg-[#F4F7F4] font-sans selection:bg-amber-200 selection:text-emerald-900">
      ...
      <aside className="hidden lg:flex inset-y-0 left-0 z-30 w-[280px] bg-gradient-to-b from-[#022c22] via-[#041e17] to-[#01140f] border-r border-emerald-800/20 flex-col shadow-2xl relative overflow-hidden">
        ...
        <div className="p-4 relative z-10 border-t border-emerald-800/20">
          <button 
            ...
            className="w-full flex items-center justify-center space-x-2 px-4 py-3 bg-emerald-900/40 hover:bg-emerald-800/60 border border-emerald-700/30 hover:border-emerald-600/50 text-emerald-100 hover:text-white rounded-2xl transition-all duration-300 text-xs font-bold group shadow-sm"
          >
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <header className="bg-emerald-950/95 backdrop-blur-md px-5 py-3.5 flex items-center justify-between z-[45] lg:hidden sticky top-0 border-b border-emerald-900/30 shadow-md">
          ...
          {!isPortalRoute && (
             <button onClick={() => setActiveApp('dashboard')} className="flex items-center justify-center p-2 bg-emerald-900/40 hover:bg-emerald-800/60 border border-emerald-700/30 rounded-xl text-emerald-100 transition-colors" title="Kembali ke Master Menu">
               <ArrowLeft size={16} />
             </button>
          )}
        </header>
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-10 pb-20 relative z-10 bg-[#f8faf9]">
            ...
            {activeTab === 'sewa' && (
              <div className="max-w-[100rem] mx-auto pb-32 lg:pb-12 pt-6 h-full flex flex-col min-h-[600px] lg:h-[calc(100vh-100px)]">
                <div className="flex flex-col md:flex-row gap-4 h-full">
                   {/* Sidebar */}
                   <div className="w-full md:w-64 lg:w-72 flex-shrink-0 bg-white rounded-3xl border border-slate-200 shadow-sm flex flex-col h-full overflow-hidden max-h-[400px] md:max-h-full">
                      <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex-shrink-0">
                         <h3 className="font-black text-xs text-slate-800 tracking-wide uppercase mb-3">Data Penyewa</h3>
                         <button
                            onClick={() => setFilterDateSewa('')}
                            className={`w-full text-left px-4 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-between ${!filterDateSewa ? 'bg-emerald-100 text-emerald-800 shadow-sm' : 'text-slate-600 hover:bg-slate-100'}`}
                         >
                            <span>Semua (Kedepan)</span>
                            <span className="bg-white/60 text-slate-500 px-2 py-0.5 rounded-full text-[10px]">{availableDatesList.total}</span>
                         </button>
                      </div>
                   ...
                   {/* Main Content */}
                   <div className="flex-1 bg-white rounded-3xl border border-slate-200 shadow-sm flex flex-col h-full overflow-hidden min-h-[500px]">
                      <div className="p-4 border-b border-slate-100 bg-emerald-950 flex items-center justify-between flex-shrink-0">
                          <h2 className="text-sm font-black text-white flex items-center gap-2">
                             <Calendar size={16} className="text-emerald-400"/> 
                             {filterDateSewa ? formatTanggalIndo(filterDateSewa) : 'Semua Jadwal Ke Depan'}
                          </h2>
                      </div>
                      
                      <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-50 p-4 sm:p-6 space-y-6">
                         {(() => {
                            const todayStr = getTodayString();
                            let dataFiltered = sewaList.filter(s => s.status_pembayaran !== 'Ditutup');
                            if (filterDateSewa) {
                               dataFiltered = dataFiltered.filter(s => s.tanggal_sewa === filterDateSewa);
                            } else {
                               dataFiltered = dataFiltered.filter(s => s.tanggal_sewa >= todayStr);
                            }
                            ...
                            if (dataFiltered.length === 0) {
                               return (
                                  <div className="flex flex-col items-center justify-center py-24">
                                    ...
                                    <p className="font-extrabold text-sm text-emerald-950">Belum ada riwayat data pengunjung.</p>
                                  </div>
                               );
                            }

                            const belumBayar = dataFiltered.filter(s => s.status_pembayaran === 'Belum Transfer');
                            const sudahBayar = dataFiltered.filter(s => s.status_pembayaran === 'Sudah Transfer' || s.status_pembayaran === 'Menunggu Verifikasi' || s.status_pembayaran === 'Sudah Lunas');

                            const renderTable = (data, title, colorClass, iconNode) => {
                               if (data.length === 0) return null;
                               return (
                                  <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-6 last:mb-0">
                                     <div className={`px-4 py-3 border-b border-slate-100 flex items-center gap-2 ${colorClass}`}>
                                        {iconNode}
                                        <h3 className="font-bold text-xs uppercase tracking-wider">{title} <span className="ml-1 bg-white/50 px-1.5 py-0.5 rounded-full text-[10px]">{data.length}</span></h3>
                                     </div>
                                        ...
                                        <table className="w-full text-left text-sm whitespace-nowrap">
                                           <thead className="bg-slate-50/60 border-b border-slate-100 text-slate-500 uppercase text-[9px] font-black tracking-widest">
                                              <tr>
                                                ...
                                                <th className="px-4 py-3">Tanggal Penyewaan</th>
                                              </tr>
                                           </thead>
                                           <tbody className="divide-y divide-slate-100 text-xs">
                                              {data.map((item, idx) => {
                                                 ...
                                                 const Icon = isLapangan ? TreePine : Tent;
                                                 const dateObj = new Date(item.tanggal_sewa);
                                                 const hari = new Intl.DateTimeFormat('id-ID', { weekday: 'long' }).format(dateObj).toUpperCase();
                                                 return (
                                                    <tr key={idx} onClick={() => handleOpenDetail(item)} className="hover:bg-slate-50/70 cursor-pointer transition-colors">
                                                       ...
                                                       <td className="px-4 py-3.5">
                                                          <span className={`px-2.5 py-1 rounded-xl text-[10px] font-black flex items-center w-max ${getStatusBadgeClass(item.status_pembayaran)}`}>
                                                             ...
                                                             {item.status_pembayaran === 'Sudah Transfer' ? 'LUNAS' : item.status_pembayaran.toUpperCase()}
                                                          </span>
                                                       </td>
                                                       <td className="px-4 py-3.5 text-slate-700 font-bold text-[10px]">{hari}</td>
                                                       <td className="px-4 py-3.5 text-slate-700 font-bold text-xs">{item.nama_penyewa}</td>
                                                       <td className="px-4 py-3.5 text-amber-600 font-bold text-[10px] max-w-[150px] truncate">{item.keterangan || '-'}</td>
                                                       <td className="px-4 py-3.5 text-slate-500 font-semibold text-xs text-blue-600">{formatTanggalPendek(item.tanggal_sewa)}</td>
                                                    </tr>
                                                 )
                                              })}
                                           </tbody>
                            ...
                            return (
                               <>
                                 {renderTable(belumBayar, "Belum Bayar", "text-rose-700 bg-rose-50", <AlertCircle size={14} className="text-rose-500"/>)}
                                 {renderTable(sudahBayar, "LUNAS", "text-emerald-700 bg-emerald-50", <CheckCircle2 size={14} className="text-emerald-500"/>)}
                               </>
                            );
                         })()}
                      </div>
